...
**Answer:** Yes, our beta 1 (V1) coefficient is biased due to omitted variable bias. By omitting V2, our beta 1 coefficient has upward bias (larger than it would be with V2 in the model: 7.87 vs 3.045). 
...
**Answer:** No, beta 1 is not biased in this model. We can see that the coefficient is very similar to model #1 where beta two was also included (3.072 vs 3.045). We can also affirm this mathematically given the relationship below
...
```
```{r}
data_vis <- plot_coefs(M1, M2, omit.coefs =c("(Intercept)","V2"), title= )
data_vis + coord_flip()
```
